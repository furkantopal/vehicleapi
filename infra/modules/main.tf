...
# Create a Project
#
resource "mongodbatlas_project" "vehicle_api" {
  name   = "VehicleApi"
...
#
resource "mongodbatlas_cluster" "cluster0" {
  project_id = mongodbatlas_project.vehicle_api.id
  name       = "Cluster0"
...
#
resource "mongodbatlas_database_user" "vehicle_api_user" {
...
resource "mongodbatlas_project_ip_access_list" "my_ipaddress" {
  ...
  comment    = var.mongodb_atlas_accesslistip
}

# Use terraform output to display connection strings.
output "connection_strings" {
  value = mongodbatlas_cluster.cluster0.connection_strings.0.standard_srv
}
